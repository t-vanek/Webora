...
## Opravy reprodukovaných instalačních chyb (2026-09-17)

- SQL preflight nově ověřuje také efektivní `CREATE DATABASE` v `master`, které vyžaduje `RESTORE VERIFYONLY`. Samotné `db_owner` cílové DB nestačilo. Dokumentace a průvodce uvádějí tento dodatečný požadavek a jeho rozsah; aplikace žádné právo sama nepřiděluje. Regrese používá vlastní izolovanou LocalDB instanci a omezený login: před grantem kontrola i ověření zálohy selžou, po minimálním grantu projdou dvě zálohy bez členství `sysadmin`/`dbcreator`.
- `Recover` může dokončit bootstrap po migraci první instalace, pokud cílové schéma přesně odpovídá, neexistuje žádný uživatel ani dokončený instalační stav a žurnál má správnou verzi, fázi a výslovné `previous: null`. Platné seed údaje jsou povinné. Kontrola nic nezapisuje, opakovaná obnova zachovává původ žurnálu a existující účet neobnovuje ani nepovyšuje. Oprava vyžaduje nové aplikační binárky, samotná výměna PowerShell skriptu staré EXE neopraví.
- Builder používá krátký unikátní pracovní adresář mimo klon a umožňuje `-WorkPath`. Původní dlouhá cesta klonu s mezerami a diakritikou nyní vytvořila celý release bez změny `LongPathsEnabled=0`.
- Příkaz Windows služby kanonizuje cesty a escapuje koncová zpětná lomítka. Nativní parser ověřuje zachování jednotlivých argumentů včetně mezery, diakritiky a kořene disku. Simulovaná obnova s koncovým lomítkem přežije selhání prvního startu a dokončí druhý pokus.

Ověřeno na Windows x64, PowerShell 7.6.6, SDK 10.0.401: 39/39 cílených .NET testů s SQL, žádný přeskočený; 72/72 PowerShell kontrol; 8/8 kontrol builderu. Nový self-contained release prošel samostatným `PublishedReleaseTests` (1/1, dva starty s PATH bez SDK, health/login stránka/statický asset a zachování šifrovaných keys). Prošla integrita manifestu a SHA-256 ZIPu i odmítnutí druhého buildu stejné verze bez přepsání souborů. Lokální testovací release byl sestaven s `-AllowDirty` a není určen pro Production/Staging.

Výsledky: `artifacts/install-fixes-dotnet.log`, `artifacts/install-fixes-powershell.log`, `artifacts/install-fixes-builder-tests.log`, `artifacts/install-fixes-published.log`; TRX v `tests/D3Parking.Application.Tests/TestResults/install-fixes*.trx`. SQL pokusy používaly vlastní testovací instance/databáze. Skutečný SCM token, privilegované ACL, veřejné produkční HTTPS a SMTP doručení tímto během ověřené nejsou; nadále vyžadují staging přejímku.
