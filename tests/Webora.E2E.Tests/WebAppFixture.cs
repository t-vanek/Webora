...
public sealed class WebAppFixture
{
    ...
    public static readonly string AdminStatePath =
        Path.Combine(Path.GetTempPath(), "webora-e2e-admin-state.json");
    ...
    private void StartApp()
    {
        ...
        _app = Process.Start(new ProcessStartInfo
        {
            FileName = "dotnet",
            Arguments =
                $"run --project src/Webora.Web/Webora.Web.csproj -c Debug --urls {BaseUrl}",
    ...
    private static string FindRepoRoot()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "Webora.slnx")))
        {
            dir = dir.Parent;
        }

        return dir?.FullName ?? throw new InvalidOperationException("Could not locate the repo root (Webora.slnx).");
    }
}
