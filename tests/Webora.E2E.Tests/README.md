...
- .NET SDK 10
- Backing services running: `docker compose up -d postgres redis rabbitmq smtp4dev`
- The Playwright browser. After the first build, install it once:

  ```bash
  pwsh tests/Webora.E2E.Tests/bin/Debug/net10.0/playwright.ps1 install chromium
  ```
...
```bash
dotnet test tests/Webora.E2E.Tests/Webora.E2E.Tests.csproj
```
...
Credentials come from `IdentitySeed` in `src/Webora.Web/appsettings.json`
